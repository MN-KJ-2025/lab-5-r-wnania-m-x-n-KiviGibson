...
def spare_matrix_Abt(m: int, n: int) -> tuple[np.ndarray, np.ndarray] | None:
    ...
    """
    if not isinstance(m, int) or not isinstance(n, int): return None
    if m < 1 or n < 1: return None
    t= np.linspace(0, 1, m)
    vec = np.cos(4*t)
    Matrix = np.vander(t, n, increasing=True)
    return Matrix, vec
...
def residual_norm(A: np.ndarray, x: np.ndarray, b: np.ndarray) -> float | None:
    ...
    if not isinstance(A, np.ndarray) or not isinstance(x, np.ndarray) or not isinstance(b, np.ndarray):
        return None
    if A.shape[1] != x.shape[0] or A.shape[0] != b.shape[0]: 
        return None
